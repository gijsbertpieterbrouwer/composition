...
import { ColorsEnum, LanguagePath, TaskFilterOptions } from "@/TickApi";
import { unref } from "vue";
import { Component, Prop, Vue } from "vue-facing-decorator";
import TaskFiltersEditor from "./TaskFiltersEditor.vue";
...
export default class TaskFiltersButton extends Vue {
  Appearance = Appearance
  Color = Color
  Size = Size
  LanguagePath = LanguagePath
  @Prop({ required: true }) private data!: TaskFilterOptions;
  ...
  private get filterIndicator(): TickIndicatorData {
    ...
    //const tooltip = nrOfFilters > 1 ?   `This list is filtered by ${nrOfFilters} filters.` : `This list is filtered by ${nrOfFilters} filter.`
    const title = nrOfFilters > 1 ? translate(LanguagePath.App_FilterIndicator_Title_Multi, [{ path: 'nr', value: nrOfFilters.toString() }]) : translate(LanguagePath.App_FilterIndicator_Title_Single, [{ path: 'nr', value: nrOfFilters.toString() }]);//`${nrOfFilters} filters` : `${nrOfFilters} filter`
  ...
  private openPanel() {
    ...
    const panelOptions: TickPanelOptions = {
      title: "TaskFiltersEditor",
      from: getRefElementDOMRect(this, "toggleButton")
    }

    this.activePanel = mount(TaskFiltersEditor, {
      props: {
  ...
  private updateFilters(newFilters?: TaskFilterOptions) {
