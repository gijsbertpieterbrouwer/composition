...
import { CollectionsData, CreateCollectionRequest, DeleteCollectionRequest, DeleteConfirmationInfo, GetAsCollectionRequest, MoveCollectionRequest, MoveObjectToCollectionRequest, UpdateCollectionRequest } from "@/TickApi";
...
export async function GetCollections(workspaceId: string) {
    ...
    try {
        const response = await Post<CollectionsData>(URL.getCollections, params);
