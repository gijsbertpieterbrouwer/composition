...
async function connect(token: string) {
  ...
  const retryPolicy: IRetryPolicy = {
    nextRetryDelayInMilliseconds: (retryContext) => {
      ...
      // Start at 5s, slow down logarithmically, max out at 30s
      const delay = Math.min(5000 * Math.log2(attempt + 2), 30000);
      debug(`[RTC:RetryPolicy] Retry attempt #${attempt + 1}, delaying for ${Math.round(delay)} ms`);
      return delay;
    },
  };
