...
interface Getters extends _GettersTree<State> {
  ...
  getEventSubscription: (state: State) => (id: string) => TickEventSubscription | null;
}

interface Actions extends _ActionsTree {
  use(): void;
  get(id: string): Promise<TickEventSubscription | null>;
  add(): Promise<string>;
...
>("eventSubscriptions", {
  ...
  getters: {
    ...
    getEventSubscription(state) {
      return (id: string) => state.eventSubscriptions.find((p) => p.id == id) ?? null;
    },
  },
  actions: {
    async delete(id) {
    ...
    async get(id): Promise<TickEventSubscription | null> {
      if (!id) { return null; }
