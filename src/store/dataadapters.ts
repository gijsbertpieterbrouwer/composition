...
interface Getters extends _GettersTree<State> {
  ...
  adapterLogDetails: (state: State) => (logId: string) => TickApiAdapterLog | null;
}

interface Actions extends _ActionsTree {
  use(): void;
  get(id: string): Promise<TickApiAdapterDefinition | null>;
...
>("dataadapters", {
  ...
  getters: {
    ...
    adapterLogDetails: (state) => (logId: string) => (state.logs || []).find((p) => p.id == logId) ?? null,
  },
  actions: {
    async delete(id) {
    ...
    async get(id): Promise<TickApiAdapterDefinition | null> {
