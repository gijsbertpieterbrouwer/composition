...
interface Getters extends _GettersTree<State> {
  dashboardData: (state: State) => (id: string) => DashboardDataResponse;
}
...
interface Actions {
  ...
  setDashboardAsWorkspaceDefauld(id: string): Promise<boolean>;
}
...
const useDashboardsStore = defineStore<"dashboardDatas", State, Getters, Actions>("dashboardDatas", {
  ...
  actions: {
    ...
    async setDashboardAsWorkspaceDefauld(id: string): Promise<boolean> {
      if (!id) { return; }
      const def = useUtilityDataStore().dashboard(id);
      if (!def) { return; }
      ...
      useUserStore().activeWorkspace.defaultDashboardId = id;
