...
interface Getters extends _GettersTree<State> {
  ...
  getFileUpload: (state: State) => (fileId: string) => UploadableFile;
}

interface Actions extends _ActionsTree {
  use(): void;
  get(id: string): Promise<TickWorkspaceFile>;
  add(): Promise<string>;
  ...
  getters: {
    ...
    getFileUpload(state) {
      return (fileId: string) =>
        (state.uploadableFiles as UploadableFile[]).find((p) => p.id == fileId);
    },
  },
  actions: {
    ...
    async get(id): Promise<TickWorkspaceFile> {
