...
interface State {
    ...
    communicatorFilters: CommunicatorFilterOptions;
    listResponse: GetCommunicatorsResponse;
    isLoading: boolean;
}

interface Getters extends _GettersTree<State> {
    getSummary: (state: State) => (id: string) => TickCommunicatorSummary;
}

interface Actions {
    use(filters: CommunicatorFilterOptions, isPaging: boolean): Promise<GetCommunicatorsResponse>;
    // addTask(request: TickCreateTaskInstanceRequest): Promise<unknown>;
    getCommunicator(id: string): Promise<TickCommunicatorWorkingData>;
    ...
    patchCommunicatorSummary(serverData: TickCommunicatorSummary): void;
}
...
const useCommunicatorsStore = defineStore<"communicators", State, Getters, Actions>("communicators", {
    ...
    getters: {
        getSummary(state) {
            return (id: string) => state.communicatorSummaries.find((p) => p.id == id);
        },
    },
    actions: {
        ...
        //   return true;
        // },
        async getCommunicator(id: string): Promise<TickCommunicatorWorkingData> {
        ...
        async updateCommunicatorDetails(id: string): Promise<FormValidationResponse[]> {
            ...
            if (!response.some(p => !p.isValid)) {
                ...
                if (summary != null) {
                    ...
                    this.patch = this.patchCommunicatorSummary(summary);
                }
            }
        ...
        async updateCommunicatorFromMetaDataComponent(ticketId: string, metadataComponentDefinitionId: string, communicatorId: string, update: MetaDataComponentCommunicatorInfoData): Promise<MetaDataComponentCommunicatorInfoData> {
            ...
            if (!response.formValidationResponse.isValid) {
                ...
                if (summary != null) {
                    summary.name = update.communicatorSummary.name;
                    summary.avatarFileId = update.communicatorSummary.avatarFileId;
                    summary.color = update.communicatorSummary.color;
                    this.patch = this.patchCommunicatorSummary(summary);
                }
            }
        ...
        patchCommunicatorSummary(serverData: TickCommunicatorSummary) {
