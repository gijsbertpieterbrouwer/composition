...
interface State {
  ...
  metaDataComponentEditorSampleData: MetaDataComponentUpdate;
...
interface Actions extends _ActionsTree {
  ...
  useUsers(): void;
  getUser(id: string): Promise<TickManagerUserData>;
  ...
  getTeam(id: string): Promise<TickManagerTeamData>;
  ...
  getEnvironment(id: string): Promise<TickExecutionEnvironment>;
  ...
  getMetaDataComponent(id: string): Promise<TickMetaDataComponent>;
  ...
  addSlaDefinition(
    env: AddSlaDefinitionRequest
  ): Promise<TickSlaDefinitionSummary>;
  getSlaDefinition(id: string): Promise<TickSlaDefinitionData>;
  saveSlaDefinition(id: string): Promise<TickSlaDefinitionData>;
  deleteSlaDefinition(id: string): Promise<unknown>;

  addTaskDefinition(): Promise<string>;
  getTaskDefinition(id: string): Promise<TickTaskDefinition>;
  saveTaskDefinition(id: string): Promise<TickTaskDefinition>;
  patchTask(serverData: TickMetaDataComponent): void;
  ...
  getCannedResponse(id: string): Promise<TickCannedResponseDefinition>;
  ...
  getKnowledgeBase(id: string): Promise<TickKnowledgeBase>;
  ...
  //activateSlackWorkspaceIntegration(id: string, code: string): Promise<TickWorkspaceIntegrationProgress>;
  deleteWorkspaceIntegration(id: string): Promise<unknown>;


  useCommunicatorCategoryDefinitions(): void;
  getCommunicatorCategoryDefinition(id: string): Promise<TickCommunicatorCategoryDefinition>;
  ...
  "managingStore",
  {
    ...
    actions: {
      ...
      patchLabel(serverData: TickLabelDefinition) {
        const index = (this.labels as TickLabelDefinition[]).findIndex(
          (s) => s.id === serverData.id
        );
        if (index === -1) {
          this.labels.push(serverData);
        } else {
          this.labels.splice(index, 1, serverData);
        }

        // add to useStore 

        // const patch: TickWorkspaceUtilityData = {
        //   labelDefinitions: [serverData]
        // }
        // useUtilityDataStore().patch(patch);
      ...
      async getUser(id: string): Promise<TickManagerUserData> {
        if (!id) {
          return;
        }
      ...
      patchUser(serverData: TickManagerUserData) {
        ...
        // const index = (this.users as TickManagerUserData[]).findIndex(
        //   (s) => s.id === serverData.id
        // );
        // if (index === -1) {
        //   this.users.push(serverData);
        // } else {
        //   this.users.splice(index, 1, serverData);
        // }
      },
      ...
      async updateUserAdmin(id: string, setAdmin: boolean): Promise<TickManagerUserData> {
        ...
        const newUserData = this.getUser(id);
        return newUserData;
      },
      ...
      async getTeam(id: string): Promise<TickManagerTeamData> {
        if (!id) {
          return;
        }
      ...
      async getEnvironment(id: string): Promise<TickExecutionEnvironment> {
        if (!id) {
          return;
        }
      ...
      async addEnvironment(
        team: AddExecutionEnvironmentRequest
      ): Promise<MetaDataItem> {
        const result = await addEnvironment(team);

        this.useEnvironments();
        return result;
      },
      async deleteEnvironment(id: string): Promise<AxiosResponse> {
      ...
      patchEnvironment(serverData: TickExecutionEnvironment) {
        ...
        // const index = (
        //   this.environments as TickExecutionEnvironment[]
        // ).findIndex((s) => s.id === serverData.id);
        // if (index === -1) {
        //   this.environments.push(serverData);
        // } else {
        //   this.environments.splice(index, 1, serverData);
        // }
      },
      ...
      async getMetaDataComponent(id: string): Promise<TickMetaDataComponent> {
        if (!id) {
          return;
        }
      ...
      patchMetaDataComponent(serverData: TickMetaDataComponent) {
        ...
        // ).findIndex((s) => s.id === serverData.id);
        // if (index === -1) {
        //   this.metaDataComponents.push(serverData);
        // } else {
        //   this.metaDataComponents.splice(index, 1, serverData);
        // }
      },
      ...
      async getSlaDefinition(id: string): Promise<TickSlaDefinitionData> {
        if (!id) {
          return;
        }
      ...
      patchSla(serverData: TickMetaDataComponent) {
      ...
      //TASKS
      async getTaskDefinition(id: string): Promise<TickTaskDefinition> {
        if (!id) {
          return;
        }
      ...
      async addTaskDefinition(): Promise<string> {
      ...
      patchTask(serverData: TickMetaDataComponent) {
      ...
      async deleteCannedResponse(id: string) {
        ...
        const summaryIndex = (this.cannedResponseSummaries as TickCannedResponseDefinition[]).findIndex((s) => s.id === id);
      ...
      async getKnowledgeBase(id: string): Promise<TickKnowledgeBase> {
        if (!id) {
          return null;
        }
      ...
      async getKnowledgeBaseArticle(id: string): Promise<TickKnowledgeBaseArticle> {
        if (!id) {
          return null;
        }
      ...
      async deleteKnowledgeBaseArticle(id: string) {
        ...
        if (index > -1) {
          this.knowledgebaseArticles.splice(this.knowledgebaseArticles, 1);
        }
      ...
      // async activateSlackWorkspaceIntegration(id: string, code: string) {
      //   const progress = await activateSlackWorkspaceIntegration(id, code);
      //   return progress;
      // },
      ...
      async getCommunicatorCategoryDefinition(id: string): Promise<TickCommunicatorCategoryDefinition> {
        if (!id) {
          return null;
        }
