...
router.beforeEach(function (to, from, next) {
  ...
  if (to.params && to.params.id) {
    debugCustom(
      `[Router:${to.name.toString()}] Navigate to: ${to.name.toString()} => ${to.params.id
      }`,
      logColors.fg.white,
...
function allowView(auth: AuthorizationObjectType): boolean {
  return useUserStore().allowedView(auth, null);
}
function tryView(auth: AuthorizationObjectType): boolean {
  if (!allowView(auth)) {
...
export function RouterToTicket(id?: string) {
