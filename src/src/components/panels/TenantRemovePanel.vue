<template>
  <TickDialogPanel class="panel" @close="close">
    ...
    <template #footer>
      <template v-if="!requestApproved">
        <TickButton v-if="busyRethinking" @click="stopRemoval" :appearance="ButtonAppearance.secondary"> {{ $translate(LanguagePath.App_TenantRemovePanel_Reconsider_Title) }}
          <TickCountdown :to="rethinkTimerToTicks" :from="rethinkTimerFromTicks" />
        </TickButton>
        <TickButton v-else :busy="busyRequesting" :disabled="!callerIsAdmin" @click="requestRemoval"> {{ $translate(LanguagePath.App_TenantRemovePanel_RemoveButton_Title) }}</TickButton>
      </template>
      <template v-else>
...
export default class TenantRemovePanel extends Vue {
  ...
  private finalAnswer = false;

  private rethinkTime = 20000;
  private rethinkTimerFromTicks: number = null;
  private rethinkTimerToTicks: number = null;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private rethinkTimerId: any = null;
  private busyRethinking = false;

  private busyRequesting = false;
  private requestApproved = false;
  ...
  private requestRemoval() {
    if (!this.callerIsAdmin) { return; }
    if (!this.finalAnswer) {
      notifyMessage(translate(LanguagePath.App_TenantRemovePanel_Remove_ConfirmationRequired_Title), translate(LanguagePath.App_TenantRemovePanel_Remove_ConfirmationRequired_Explanation))
      return;
    }
    // if (!confirm("Are you sure? This action has permanent implications.")) { return; }
