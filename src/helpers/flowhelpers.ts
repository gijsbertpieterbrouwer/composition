  ...
  flow?.steps?.forEach((step) => {
    ...
    if (step.targetStepId) {
      connections.push({
        ...
        animated: stepTravelData?.some(p => p.sourceActionId == null && p.targetStepId == step.targetStepId) ?? false,
      });
    }
            ...
            if (questionSettings.awaitResponseSettings != null && questionSettings.awaitResponseSettings.failureTargetStepId) {
              connections.push({
                ...
                animated: actionTravelData?.some(p =>
                  ...
                  && p.targetStepId == questionSettings?.awaitResponseSettings?.failureTargetStepId) ?? false,
              });
            }
                ...
                if (route.targetStepId) {
                  connections.push({
                    ...
                    animated: actionTravelData?.some(p => p.sourceActionOptionId == route.id
                      && p.targetStepId == route.targetStepId) ?? false,
                  });
                }
              });
            }
          ...
          if (settings.actionType == BotActionTypeEnum.SearchKnowledgeBase) {
            ...
            if (searchKnowledgeBaseSettingsSettings.awaitResponseSettings != null && searchKnowledgeBaseSettingsSettings.awaitResponseSettings.failureTargetStepId) {
              connections.push({
                ...
                color: getColorHex(ColorsEnum.Red),
                animated: actionTravelData?.some(p => p.targetStepId == searchKnowledgeBaseSettingsSettings?.awaitResponseSettings?.failureTargetStepId) ?? false,
              });
            }
            ...
            if (searchKnowledgeBaseSettingsSettings.bypassTargetStepId != null) {
              connections.push({
                ...
                animated: actionTravelData?.some(p => p.sourceActionOptionId == bypass && p.targetStepId == searchKnowledgeBaseSettingsSettings.bypassTargetStepId) ?? false,
              });
            }
          }
          ...
          if (settings.actionType == BotActionTypeEnum.Choice) {
            ...
            if (choiceSettings.awaitResponseSettings != null && choiceSettings.awaitResponseSettings.failureTargetStepId) {
              connections.push({
                ...
                animated: actionTravelData?.some(p =>
                  p.sourceActionOptionId == failure
                  && p.targetStepId == choiceSettings?.awaitResponseSettings?.failureTargetStepId) ?? false,
              });
            }
              ...
              choiceSettings?.options?.forEach((route) => {
                if (route.targetStepId) {
                  connections.push({
                    ...
                    targetStep: route.targetStepId,
                    color: getColorHex(ColorsEnum.Yellow),
                    animated: actionTravelData?.some(p => p.sourceActionOptionId == route.id && p.targetStepId == route.targetStepId) ?? false,
                  });
                }
              });
            }
        ...
        case FlowActionType.DataAdapter: {
          ...
          if (dataAdapterSettings.failTargetStepId) {
            connections.push({
              ...
              color: getColorHex(ColorsEnum.Red),
              animated: actionTravelData?.some(p => p.sourceActionOptionId == failure && p.targetStepId == dataAdapterSettings.failTargetStepId) ?? false,
            });
          }
              ...
              taskSettings.actionRoutes.forEach((route) => {
                if (route.targetStepId) {
                  ...
                  connections.push({
                    ...
                    sourceStep: step.id,
                    sourceActionId: action.id,
                    sourceActionOption: route.taskActionId ?? undefined,
                    target: getConnectionHandleId(route.targetStepId),
                    targetStep: route.targetStepId,
                    color: color,
                    animated: actionTravelData?.some(p => p.sourceActionOptionId == route.taskActionId && p.targetStepId == route.targetStepId) ?? false,
                  });
                }
              });
            }
        ...
        case FlowActionType.Special: {
          ...
          if (specialSettings.failTargetStepId) {
            connections.push({
              ...
              animated: actionTravelData?.some(p =>
                p.sourceActionOptionId == failure
                && p.targetStepId == specialSettings.failTargetStepId) ?? false,
            });
          }
...
export function connect(
  ...
  if (source.actionId) {
    ...
    if (!source.optionId) {
      ...
      const actionSettings = jsonProxy<{ actionDataJson: string, actionType: number }>(action?.actionSettingsJson || "{}",
        (json) => {
          if (action) {
            action.actionSettingsJson = json;
          }
      ...
      switch (action?.actionType) {
        ...
        case FlowActionType.Task: {
          ...
          const route = taskSettings?.actionRoutes?.find((p) => p.taskActionId == source.optionId);
          if (route) {
            route.targetStepId = target.stepId;
          }
          //fixme: proxy needs to see array changes
...
export function disconnect(
  ...
  if (source.actionId) {
    ...
    if (!source.optionId) {
      const settings = jsonProxy<{ targetStepId: string | null }>(
        action?.actionSettingsJson || "{}",
        (json) => {
          if (action) {
            action.actionSettingsJson = json;
      ...
      switch (action?.actionType) {
        ...
        case FlowActionType.DataAdapter: {
          const dataAdapterSettings = jsonProxy<{ failTargetStepId: string | null }>(
            actionSettings.actionDataJson || "{}",
            (json) => {
              actionSettings.actionDataJson = json;
            }
...
export function patchFlowMetadata(flow: TickFlowDefinition, delta: Partial<TickFlowDefinitionMetaData>) {
  const source: Required<TickFlowDefinitionMetaData> = {
    ...
    startStepId: flow.startStepId ?? null,
    settingsNodePosition: flow.settingsNodePosition,
    startConditionsJson: flow.startConditionsJson ?? null,
    skipConditionsJson: flow.skipConditionsJson ?? null,
    startOnCommunicationChannelIds: flow.startOnCommunicationChannelIds ?? null,
    defaultBotUserId: flow.defaultBotUserId ?? null,
